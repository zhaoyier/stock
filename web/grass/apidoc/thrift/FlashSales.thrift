...
enum ExceptionErrCode {
  Error,
  NotFound,
  ProductStockNotEnough,
}

exception FlashSalesAPIException {
  1:required ExceptionErrCode errCode
  2:required string errMessage
}

struct TFlashSalesProductDetail {
  1:required double  productPrice;//商品价格
  2:required i64     salesStartTime;//闪购开始时间,时间戳
  3:required i64     salesEndTime;//闪购结束时间,时间戳
  4:required i32     salesLimit;//购买数量限制
  5:required i32     productStock;//当前商品库存数量
  6:required list<string> areas;//商品属于哪些国家
}
...
struct TFlashSalesProduct {
	...
	13:required list<string> areas; //闪购商品支持的区域(国家)
        14:required i64 gpid; //商品的gpid
}

struct TFlashSalesProductWithActivityTime {
	1:required string url;	//商品标准URL 格式 https://item.taobao.com/item.htm?id=xxxx
	2:required string localPrice; //商品的闪购本地价格
	3:optional string originLocalPrice; //商品的闪购原价
	4:optional double discount; //闪购折扣比例
	5:required string image; //商品图片
	6:required i32 activityStartTime; //闪购活动开始时间
}
...
service FlashSales {
	...
	// 添加闪购活动的top 商品，首页展示
	TResult UserSetFlashSalesSettingTopProduct(1:string settingId,2:string productUrl),
	
	// 显示闪购活动top商品url
	string UserGetFlashSalesSettingTopProductUrl(1:string settingId),
	...
	/// 获取一天的闪购活动
	/// date: 具体日期（2016-06-15）
	/// top: 前几个商品（0表示不取商品），如果这几个商品都售罄，则取后面几个
	/// </summary>
	list<TFlashSalesSummary> GetFlashSalesListByDateWithoutSoldoutAll(1:string date, 2:i32 top, 3:string area),
	...
	// 获取首页闪购入口展示的闪购商品，返回有1个或0个闪购商品列表
	list<TFlashSalesCategoryProducts> GetFlashSalesListForHomepage(1:string area),

	// 获取双十一促销页面的闪购商品,返回4个hot商品
	list<TFlashSalesCategoryProducts> GetFlashSalesListForPromotionPage(1:string area),

	// 获取双十二促销页面的闪购商品
	list<TFlashSalesProductWithActivityTime> GetHotProductWithActivityTimeList(1:i32 limit),

  	//获取商品详情页
  TFlashSalesProductDetail GetFlashSalesProductDetail(1:string settingId, 2:string refId) throws (1:FlashSalesAPIException exception),
	
	//增加商品库存
	bool AddProductStock(1:string settingId, 2:string refId, 3:i32 num) throws (1:FlashSalesAPIException exception),

	//减少商品库存
	bool ReduceProductStock(1:string settingId, 2:string refId, 3:i32 num) throws (1:FlashSalesAPIException exception)
}
