...
struct TOrder{
	...
	8:required i32 packageId;				//包裹号id
	9:required string altProductName;		//商品英文名称
	10:required string sku;					//sku名称
	11:required string altSku;				//sku英文名称
	12:required string shipmentTypeName;	//运输方式名称
	13:required string altShipmentTypeName;	//运输方式英文名称
	14:required string shipmentTypeId;		//运输方式id
	15:required double unitPrice;			//单价
	16:required double localUnitPrice;		//本地货币单价
	17:required i32 qty;					//商品数量
	18:required string warehouseCode;		//仓库
	19:required string orderStatus;			//订单状态
}
...
struct TOrderDetail {
	...
	24:required bool isOrderStatusHighlight;	//订单状态是否需要高亮
	25:optional string promotion;				//促销活动
	26:required Common.TServiceType serviceType;		// 服务类型
	27:optional TShip4MeOrderDetailInfo ship4MeInfo;
	29:optional string groupbuyEventId; // 如果是拼团,会提供大团id
	30:optional string groupId; // 如果是拼团,会提供小团id
	31:optional list<PrimeOrder.TTitleValueItem> paymentInfo;
	32:required i64 gpid;
}

struct TShip4MeOrderDetailInfo {
	1:required list<PrimeOrder.TTitleValueItem>  courierInfo; //快递信息
	2:optional list<string> takenPhotos;
	3:optional string repackService;
	4:required i32 storageDays;
...
#struct TOrderProcessing{
#	1:required string orderType;			//订单类型 buy4me,ship4me,ezbuy,prime
#	2:required  ;					//排列顺序
#	3:required list<TOrder> orders;			//此状态下的最后3个订单
#
#}
...
service Order {
	...
	/// <summary>
	/// 获取用户处于processing状态的订单
	/// </summary>
	//list<TOrderProcessing> UserGetProcessingOrders(1:i32 offset, 2:i32 limit),
	
	/// <summary>
	/// 保存前台备注信息
	/// </summary>
	/// <returns>错误信息，空为成功</returns>
	string UserSaveNewOrderFrontRemark(1:i32 orderId, 2:i32 orderRemarkId, 3:string remarkText),
	
	/// <summary>
	/// 保存前台备注信息
	/// </summary>
	/// <returns>错误信息，空为成功</returns>
	string UserSaveNewOrderFrontRemarkWithAttachment(1:i32 orderId, 2:i32 orderRemarkId, 3:string remarkText, 4:string pictures),
	
	/// <summary>
	/// 对订单的差额补齐回复一个前台备注
	/// </summary>
	/// <returns>错误信息，空为成功</returns>
	string UserReplyOffsetInOrderRemark(1:i32 orderId, 2:string remarkText),
