...
class SigninPage2 extends React.Component<{}, SigninPageState> {
	constructor(props) {
		super(props);
		this.state = {
			checked: true,
	...
	login() {
		...
		if (!this.validatePhone(username)) {
			return message.warn(
				getText("The current mobile number is bound to multiple stores, please use the login name!")
			);
		}
		...
		AccountSignin(username, password).then((data: AccountBase) => {
	...
	async validatePhone(phone) {
		return await new Promise((res, rej) => {
			if (phonePattern.test(phone)) {
				AccountCanSigninByPhone(phone).then(result => {
	...
	render() {
		...
		return (
			<div>
				<Header active={SellerTitleKey.LOGIN} forceUpdate={() => this.forceUpdate()} />
					...
					<div className="container">
						<div className="login">
							<div className="login__form">
								...
								<div className="login_userName_info">
									{" "}
									{!phoneCanSignin &&
										getText(
											"The current mobile number is bound to multiple stores, please use the login name!"
										)}{" "}
								</div>
								<div className="login__form__item">
								...
								<div className="login__form__info">
									...
									<a
										className="sign-document"
										onClick={() => {
											// this.setState({visible:true});
											const url =
												lang === "zh"
													? "/asset/ezbuysellerterms.html"
													: "https://docs.google.com/document/d/e/2PACX-1vSTdlgE6IXLcm6zr7YcMte6m1X43HRCecVUu1zqpXFJEpmyl6b67MESy0OdeLRA-s4ppmK2Tozcy5T4/pub";
								...
								<div className="login__form__tag">
									<a href={`${window.location.pathname}#forgetPassword`}>
