...
ColorPicker = (function(superClass) {
  ...
  function ColorPicker() {
    ...
    dom(this.container).addClass('old-ql-color-picker');
  }
...
Wrapper = (function() {
  ...
  Wrapper.prototype.data = function(key, value) {
    var ref;
    if (value != null) {
      if (this.node['old-ql-data'] == null) {
        this.node['old-ql-data'] = {};
      }
      this.node['old-ql-data'][key] = value;
      return this;
    } else {
      return (ref = this.node['old-ql-data']) != null ? ref[key] : void 0;
    }
  };
...
Picker = (function() {
  Picker.TEMPLATE = '<span class="old-ql-picker-label"></span><span class="old-ql-picker-options"></span>';

  function Picker(select) {
    this.select = select;
    this.container = document.createElement('span');
    this.buildPicker();
    dom(this.container).addClass('old-ql-picker');
    ...
    dom(this.label).on('click', (function(_this) {
      return function() {
        _.defer(function() {
          return dom(_this.container).toggleClass('old-ql-expanded');
        });
        return false;
      };
    })(this));
    dom(this.select).on('change', (function(_this) {
      return function() {
        ...
        if (_this.select.selectedIndex > -1) {
          item = _this.container.querySelectorAll('.old-ql-picker-item')[_this.select.selectedIndex];
          option = _this.select.options[_this.select.selectedIndex];
        }
        _this.selectItem(item, false);
        return dom(_this.label).toggleClass('old-ql-active', option !== dom(_this.select)["default"]());
      };
    })(this));
  }
  ...
  Picker.prototype.buildItem = function(picker, option, index) {
    ...
    dom(item).addClass('old-ql-picker-item').text(dom(option).text()).on('click', (function(_this) {
      return function() {
  ...
  Picker.prototype.buildPicker = function() {
    ...
    this.label = this.container.querySelector('.old-ql-picker-label');
    picker = this.container.querySelector('.old-ql-picker-options');
  ...
  Picker.prototype.close = function() {
    return dom(this.container).removeClass('old-ql-expanded');
  };

  Picker.prototype.selectItem = function(item, trigger) {
    var selected, value;
    selected = this.container.querySelector('.old-ql-selected');
    if (selected != null) {
      dom(selected).removeClass('old-ql-selected');
    }
    if (item != null) {
      value = item.getAttribute('data-value');
      dom(item).addClass('old-ql-selected');
...
Authorship = (function() {
  ...
  Authorship.prototype.attachButton = function(button) {
    ...
    return $button.on('click', (function(_this) {
      return function() {
        $button.toggleClass('old-ql-on');
        return _this.enable($dom.hasClass('old-ql-on'));
      };
    })(this));
  };
...
ImageTooltip = (function(superClass) {
  ...
  ImageTooltip.DEFAULTS = {
    template: '<div class="editorUpload"><button type="button" id="quillUpload" class="ant-btn ant-btn-ghost"><i class="anticon anticon-upload"></i><span>上传图片</span></button><span>(拖拽调整图片展示排序)</span></div><ul id="customImageList"></ul><input style="display:none;" class="input" id="editorInput" type="textbox"> <div style="display: none;" class="preview"> <span>Preview</span> </div> <a href="javascript:;" class="cancel">Cancel</a> <a href="javascript:;" class="insert">Insert</a>'
  };
  ...
  function ImageTooltip(quill, options) {
    ...
    dom(this.container).addClass('old-ql-image-tooltip');
    this.initListeners();
  }

  ImageTooltip.prototype.initListeners = function() {
    var self = this;
    dom(this.quill.root).on('focus', function() {
      setTimeout(function(){
        self.container.style.top = 0;
      } , 0);
      _.bind(self.hide, self)
    });
    ...
    dom(this.textbox).on('stupid_input', _.bind(this._preview, this));
  ...
  ImageTooltip.prototype.insertImage = function() {
    var self = this;
    var imageListContainer = document.querySelector("#customImageList");
    var imageList = imageListContainer.childNodes;
    for (i = imageList.length - 1; i >= 0 ; i--) {
      insert(imageList[i].getAttribute("url"));
    }
    imageListContainer.innerHTML = "";
    function insert(src) {
      var index, url;
      url = self._normalizeURL(src);
      if (self.range == null) {
        self.range = new Range(0, 0);
      }
      if (self.range) {
        self.preview.innerHTML = '<span>Preview</span>';
        // self.textbox.value = '';
        index = self.range.end;
        self.quill.insertEmbed(index, 'image', url, 'user');
        self.quill.setSelection(index + 1, index + 1);
      }
    }
    return this.hide();
  };
...
LinkTooltip = (function(superClass) {
  ...
  function LinkTooltip(quill, options) {
    ...
    dom(this.container).addClass('old-ql-link-tooltip');
    this.textbox = this.container.querySelector('.input');
...
MultiCursor = (function(superClass) {
  ...
  function MultiCursor(quill, options) {
    ...
    this.options = options;
    this.cursors = {};
    this.container = this.quill.addContainer('old-ql-multi-cursor', true);
...
PasteManager = (function() {
  ...
  function PasteManager(quill, options) {
    ...
    this.container = this.quill.addContainer('old-ql-paste-manager');
...
Toolbar = (function() {
  ...
  function Toolbar(quill, options) {
    ...
    dom(this.container).addClass('old-ql-toolbar');
    if (dom.isIOS()) {
  ...
  Toolbar.prototype.initFormat = function(format, callback) {
    ...
    return dom(input).on(eventName, (function(_this) {
      return function() {
        var range, value;
        value = eventName === 'change' ? dom(input).value() : !dom(input).hasClass('old-ql-active');
  ...
  Toolbar.prototype.setActive = function(format, value) {
    ...
    if (input.tagName === 'SELECT') {
      ...
      return $input.toggleClass('old-ql-active', value || false);
    }
  };
...
Tooltip = (function() {
  ...
  function Tooltip(quill, options) {
    this.quill = quill;
    this.options = options;
    this.container = this.quill.addContainer('old-ql-tooltip');
...
Quill = (function(superClass) {
  ...
  function Quill(container1, options) {
    ...
    this.options.id = this.id = "old-ql-editor-" + (Quill.editors.length + 1);
    this.modules = {};
    this.root = this.addContainer('old-ql-editor');
...
module.exports = ".old-ql-image-tooltip{padding:10px;width:300px}.old-ql-image-tooltip:after{clear:both;content:\"\";display:table}.old-ql-image-tooltip a{border:1px solid #000;box-sizing:border-box;display:inline-block;float:left;padding:5px;text-align:center;width:50%}.old-ql-image-tooltip img{bottom:0;left:0;margin:auto;max-height:100%;max-width:100%;position:absolute;right:0;top:0}.old-ql-image-tooltip .input{box-sizing:border-box;width:100%}.old-ql-image-tooltip .preview{margin:10px 0;position:relative;border:1px dashed #000;height:200px}.old-ql-image-tooltip .preview span{display:inline-block;position:absolute;text-align:center;top:40%;width:100%}.old-ql-link-tooltip{padding:5px 10px}.old-ql-link-tooltip input.input{width:170px}.old-ql-link-tooltip a.done,.old-ql-link-tooltip input.input{display:none}.old-ql-link-tooltip a.change{margin-right:4px}.old-ql-link-tooltip.editing a.done,.old-ql-link-tooltip.editing input.input{display:inline-block}.old-ql-link-tooltip.editing a.change,.old-ql-link-tooltip.editing a.remove,.old-ql-link-tooltip.editing a.url{display:none}.old-ql-multi-cursor{position:absolute;left:0;top:0;z-index:1000}.old-ql-multi-cursor .cursor{margin-left:-1px;position:absolute}.old-ql-multi-cursor .cursor-flag{bottom:100%;position:absolute;white-space:nowrap}.old-ql-multi-cursor .cursor-name{display:inline-block;color:#fff;padding:2px 8px}.old-ql-multi-cursor .cursor-caret{height:100%;position:absolute;width:2px}.old-ql-multi-cursor .cursor.hidden .cursor-flag{display:none}.old-ql-multi-cursor .cursor.top .cursor-flag{bottom:auto;top:100%}.old-ql-multi-cursor .cursor.right .cursor-flag{right:-2px}.old-ql-paste-manager{left:-100000px;position:absolute;top:50%}.old-ql-toolbar{box-sizing:border-box}.old-ql-tooltip{background-color:#fff;border:1px solid #000;box-sizing:border-box;position:absolute;top:0;white-space:nowrap;z-index:2000}.old-ql-tooltip a{cursor:pointer;text-decoration:none}.old-ql-container{box-sizing:border-box;cursor:text;font-family:Helvetica,Arial,sans-serif;font-size:13px;height:100%;line-height:1.42;margin:0;overflow-x:hidden;overflow-y:auto;padding:12px 15px;position:relative}.old-ql-editor{box-sizing:border-box;min-height: 300px;height:100%;outline:0;tab-size:4;white-space:pre-wrap}.old-ql-editor div{margin:0;padding:0}.old-ql-editor a{text-decoration:underline}.old-ql-editor b{font-weight:700}.old-ql-editor i{font-style:italic}.old-ql-editor s{text-decoration:line-through}.old-ql-editor u{text-decoration:underline}.old-ql-editor a,.old-ql-editor b,.old-ql-editor i,.old-ql-editor s,.old-ql-editor span,.old-ql-editor u{background-color:inherit}.old-ql-editor img{max-width:100%}.old-ql-editor blockquote,.old-ql-editor ol,.old-ql-editor ul{margin:0 0 0 2em;padding:0}.old-ql-editor ol{list-style-type:decimal}.old-ql-editor ul{list-style-type:disc}.old-ql-editor.old-ql-ie-10 br,.old-ql-editor.old-ql-ie-9 br{display:none}";
...
BaseTheme = (function() {
  ...
  function BaseTheme(quill, options) {
    ...
    this.quill = quill;
    this.options = options;
    dom(this.quill.container).addClass('old-ql-container');
    if (this.options.styles) {
      this.addStyles(baseStyles + BaseTheme.objToCss(this.options.styles));
    }
    if (dom.isIE(10)) {
      version = dom.isIE(9) ? '9' : '10';
      dom(this.quill.root).addClass('old-ql-ie-' + version);
    }
  }
...
SnowTheme = (function(superClass) {
  ...
  function SnowTheme(quill, options) {
    ...
    dom(this.quill.container).addClass('old-ql-snow');
    this.pickers = [];
    this.quill.on(this.quill.constructor.events.SELECTION_CHANGE, (function(_this) {
      return function(range) {
        if (range != null) {
  ...
  SnowTheme.prototype.extendToolbar = function(module) {
    dom(module.container).addClass('old-ql-snow');
    _.each(['color', 'background', 'font', 'size', 'align'], (function(_this) {
      return function(format) {
        var picker, select;
        select = module.container.querySelector(".old-ql-" + format);
        if (select == null) {
          return;
        }
        switch (format) {
          ...
          case 'background':
            ...
            _.each(picker.container.querySelectorAll('.old-ql-picker-item'), function(item, i) {
              if (i < 7) {
                return dom(item).addClass('old-ql-primary-color');
              }
            });
        }
        if (picker != null) {
